...
from kzsj import KuzushijiDataset, MultiScaleInferenceKuzushijiDataset, load_gt
    ...
    with th.no_grad():
        for ret in tqdm(loader):
            ...
            for j in range(len(hm_pred)):
                ...
                # classes_preds = classes_preds[:, :4328]
                classes_preds = classes_preds[:, :4328]
...
if __name__ == '__main__':
    config = Config()
    config.train_dir = 'data/full_dataset'
    config.test_dir = 'data/full_dataset'
    config.batch_size = 2
    config.fold = 0
    config.num_folds = 10
    config.device = 'cuda'
    ...
    config.train_csv = 'data/full_dataset/train.csv'
    config.slug = 'r101d'
    config.weight = 'checkpoints/f00-ep-0125-val_hm_acc-0.9944-val_classes_acc-0.4986.pth'  # noqa
